...
public class RangeBarChart extends BarChart {
    ...
    public void drawSeries(Canvas canvas, Paint paint, List<Float> points,
        ...
        if (halfDiffX < 1.0f) {
            halfDiffX = (float)mRenderer.getBarSpacing()*0.5f;
        }
        int start = 0;
        /*
        if (startIndex > 0) {
...
System.err.println("not enough points. size = "+points.size());
            }
  */
        }
        paint.setColor(seriesRenderer.getColor());
    ...
    protected void drawChartValuesText(Canvas canvas, XYSeries series, XYSeriesRenderer renderer,
        ...
        */
        for (int i = start; i < points.size(); i += 4) {
            ...
            if ((index+1) < series.getItemCount() &&
                    !isNullValue(series.getY(index + 1)) && points.size() > i + 3) {
